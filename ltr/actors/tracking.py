...
class HCATActor(BaseActor):
    ...
    def __call__(self, data):
        """
        args:
            data - The input data, should contain the fields 'test_images', 'train_images', 'test_anno'.
        ...
        """
        import time
        outputs = self.net(data['test_images'], data['train_images'])

        # generate labels
        targets =[]
        targets_origin = data['test_anno']
        _, _, h, w = data['test_images'].shape
        ...
        for i in range(len(targets_origin)):
            ...
            label = np.array([0])
            label = torch.tensor(label, device=data['test_anno'].device)
            target['labels'] = label
