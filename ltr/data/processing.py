...
class TransTProcessing(BaseProcessing):
    ...
    def _get_jittered_box(self, box, mode):
        ...
        args:
            box - input bounding box
            mode - string 'search' or 'template' indicating test or train data
    ...
    def __call__(self, data: TensorDict):
        ...
        args:
            data - The input data, should contain the following fields:
                'test_images', train_images', 'test_anno', 'train_anno'
        returns:
            TensorDict - output data block with following fields:
                'test_images', 'train_images', 'test_anno', 'train_anno'
        """
        ...
        if self.transform['joint'] is not None:
            ...
            data['train_images'], data['train_anno'] = self.transform['joint'](image=data['train_images'], bbox=data['train_anno'], new_roll=False)

        for s in ['test', 'train']:
            assert self.mode == 'sequence' or len(data[s + '_images']) == 1, \
                "In pair mode, num test/train frames must be 1"
        ...
        if self.mode == 'sequence':
            ...
            data = data.apply(lambda x: x[0] if isinstance(x, list) else x)
        data['train_images'] = data['train_images'].squeeze()
        data['test_images'] = data['test_images'].squeeze()
        data['train_anno'] = data['train_anno'].squeeze()
        data['test_anno'] = data['test_anno'].squeeze()
        return data
